...
export const QtyView = ({ count, addProductToCart, subtractProductFromCart }) => (
  <div className="d-flex align-item-center my-1">
    <button className="qty-view-btn border-red mr-1" onClick={subtractProductFromCart}>
      -
    </button>
    <div className="font-size-5 font-weight-5 mr-1">{count}</div>
    <button className="qty-view-btn border-green" onClick={addProductToCart}>
      +
    </button>
  </div>
);
    ...
    return (
      <div className="container">
        ...
        {this.props.cart.length !== 0 ? (
            ...
            <div className="cart-card font-weight-4 my-1" key={item._id}>
              ...
              <div className="f-3">
                ...
                <div className="card-product-false-mrp">
                  MRP: &#8377; <s>{item.productMRP}</s>
                </div>
                <div className="card-product-mrp font-size-5">
                  Discounted price: &#8377;{' '}
                  {Math.floor(
                    item.productMRP - (item.productMRP * item.productDiscountInPercent) / 100
                  )}
                </div>
              </div>
              <div className="font-weight-5 font-size-5">
                ...
                <QtyView
                  addProductToCart={() => this.props.addProductToCart(item)}
          ...
          <div className="text-center my-3 font-weight-bold font-size-3">
            Ah Oh! You cart is empty! Fill some grocery! :(
          </div>
        )}
